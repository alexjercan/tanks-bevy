...
fn setup(
    ...
    let init_vel = SetVelocitySphereModifier {
        center: writer.lit(Vec3::ZERO).expr(),
        speed: (writer.rand(ScalarType::Float) * writer.lit(2.) + writer.lit(6.)).expr(),
    };
    ...
    let impact_handle = effects.add(effect);
...
fn play_shell_impact(
    ...
    for event in impacts.read() {
        commands.spawn((
            Name::new("Impact"),
            ParticleEffectBundle {
                effect: ParticleEffect::new(particle_systems.impact.clone()),
